...
export function buildPolicySystemHint(
  ...
  return [
    ...
    knowledgePacket && !knowledgePacket.canDirectAnswer && knowledgePacket.metadata?.needsVet !== true
      // 只约束"确定性"，不约束"信息量"。之前这里禁止一切病因猜测，结果用户
      // 只想要个大概方向也拿不到，回答显得处处不沾锅。
      ? `- 普通无证据边界：当前没有可引用的对应资料，但要正常回答。可以结合通用养宠知识列出 2-4 个常见可能方向，并写清各自的典型伴随表现和对应的居家处理思路；措辞必须是可能性（“常见的可能有…”“如果同时还有…那更像…”），不要写成确定结论（禁止“大概率就是/肯定是某病”）。不得给出用药名称或剂量、不得伪造来源、不要把普通问题直接升级为急诊。`
      : null,
    ...
    knowledgePacket && !knowledgePacket.canDirectAnswer
      ...
      ? [
          ...
          '  ② 结合通用养宠知识说说常见的可能方向（写成可能性，不下结论），并给出该场景下具体到能照着做的照护要点（环境温度、喂养节奏、清洁、观察频率等）；',
          '  ③ 列出需要立刻就医的红旗信号（写清楚是什么表现）；',
          '  ④ 结尾提示尽快找兽医当面评估，并注明线上建议不能替代面诊。',
          '  禁止：把可能性写成确定诊断、给用药名称或剂量、伪造资料来源；也不要给催吐/灌药这类可能加重病情的居家操作。',
        ].join('\n')
      : null,
