// 共享组件：Header、CartDrawer、ProductCard、ArticleCard 等
...
// —— Header ——
function Header({ route, navigate, cartCount, onCartOpen }) {
  const navItems = [
    { id: 'home',     label: '首页' },
    { id: 'shop',     label: '商品' },
    ...
    <header style={{
      ...
      <div className="container" style={{
        ...
        <button onClick={() => navigate({ page: 'home' })}
                style={{ border: 0, background: 'transparent', color: 'var(--ink)', padding: 0 }}>
          <Logo />
        </button>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button onClick={onCartOpen} className="btn btn-line btn-sm"
                  style={{ position: 'relative', paddingLeft: 14, paddingRight: 14 }}>
    ...
    <footer style={{ background: 'var(--ink)', color: 'rgba(255,255,255,.7)', padding: '80px 0 32px' }}>
      <div className="container">
        ...
        <div style={{
          ...
          <span className="mono">v 2.6 · 上海 → 你家门口</span>
        </div>
      </div>
    </footer>
  );
...
// —— ProductCard ——
function ProductCard({ p, onOpen, onAdd, compact }) {
  return (
    <div className="card fade-up" style={{ padding: 12, cursor: 'pointer' }} onClick={() => onOpen(p)}>
      ...
      <div style={{ padding: '14px 6px 6px' }}>
        <div style={{ fontSize: 14, fontWeight: 600, lineHeight: 1.35, minHeight: 38, textWrap: 'pretty' }}>
          {p.name}
        </div>
...
// —— ArticleCard ——
function ArticleCard({ a, onOpen, featured }) {
  return (
    <article className="card fade-up" onClick={() => onOpen(a)}
      ...
      <div style={{ padding: featured ? '40px' : '20px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <h3 style={{
          fontSize: featured ? 28 : 18, fontWeight: 600,
          letterSpacing: '-0.01em', lineHeight: 1.25,
          margin: '0 0 12px', textWrap: 'pretty',
        }}>{a.title}</h3>
        <p className="body" style={{ margin: 0, fontSize: featured ? 15 : 14,
...
// —— CartDrawer ——
function CartDrawer({ open, onClose, items, setQty, removeItem, onCheckout }) {
    ...
    <>
      {open && (
        <div onClick={onClose} style={{
          position: 'fixed', inset: 0, background: 'rgba(38,70,83,.35)',
          zIndex: 60, animation: 'fadeBg .2s ease',
        }}/>
      )}
      <aside style={{
        ...
        boxShadow: '-12px 0 40px -8px rgba(38,70,83,.18)',
      }}>
        <div style={{
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          padding: '24px 28px', borderBottom: '1px solid var(--line-2)',
        }}>
          <div>
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
              {items.map(it => (
                <li key={it.id} style={{
                  ...
                  padding: '16px 0', borderBottom: '1px solid var(--line-2)',
                }}>
                  <div style={{
                    width: 72, height: 72, borderRadius: 14, background: it.bg,
                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 8 }}>
                      <div style={{
                        ...
                        border: '1px solid var(--line)', overflow: 'hidden', height: 26,
                      }}>
                        <button onClick={() => setQty(it.id, Math.max(0, it.qty - 1))}
                                style={{ width: 26, height: 26, border: 0, background: 'transparent', color: 'var(--ink)' }}>−</button>
                      ...
                      <button onClick={() => removeItem(it.id)}
                              style={{ background: 'transparent', border: 0, padding: 0, color: 'var(--ink-3)', fontSize: 12 }}>
                        删除
                      </button>
                    </div>
                  </div>
        ...
        {items.length > 0 && (
          <div style={{ padding: 28, borderTop: '1px solid var(--line-2)', background: 'var(--surface)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline',
                          fontSize: 14, fontWeight: 600, marginTop: 12, paddingTop: 12,
                          borderTop: '1px solid var(--line-2)' }}>
              <span>合计</span>
              <span className="mono" style={{ fontSize: 24, letterSpacing: '-0.01em' }}>{fmt(subtotal + shipping)}</span>
            </div>
            <button onClick={onCheckout} className="btn btn-primary btn-lg" style={{ width: '100%', marginTop: 16, justifyContent: 'center' }}>
              去结算 →
            </button>
          </div>
        )}
...
Object.assign(window, { fmt, Logo, Header, Footer, ProductCard, ArticleCard, CartDrawer });
